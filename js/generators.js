export function generatePublicationMarkdown(entry) {
    ...
    const authors = entry.authors.filter((a) => a.surname || a.name).map((a) => `${a.surname}, ${a.name}`).join("; ");
...
// --- NEW GENERATOR FOR JOURNAL ISSUES ---
export function generateJournalMarkdown(entry) {
    let md = "";
    
    // Line 1: Bibliographic Info
    if (entry.openAccess) {
        md += `<span style="background-color: #5a96d0; color: white; padding: 0.25em 0.4em; border-radius: 0.25rem; font-size: 75%; line-height: 1;">Open Access</span> `;
    }
...
export function generateEventMarkdown(entry) {
    ...
    if (entry.dateStart) md += `Dates: ${entry.dateStart}${entry.dateEnd ? " to "+entry.dateEnd : ""}  \n`;
    if (entry.place) md += `Place: ${entry.place}${entry.venue ? ", "+entry.venue : ""}  \n`;
...
export function generateMediaMarkdown(entry) {
    let md = "";
    if (entry.title) md += `**${entry.title}** (${entry.mediaType || "Media"})  \n`;
